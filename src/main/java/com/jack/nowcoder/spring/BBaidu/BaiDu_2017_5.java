...
public class BaiDu_2017_5 {
    /**
     * dp[i][j]表示有i个数字和j个小于号所能组成的数量，大于号数量为i-j-1次
     * 当加入第i+1个数字时分为四种情况
     * （1）如果i+1插入当前序列开头相当于同时加入一个大于号
     * （2）如果i+1插入当前序列末尾相当于同时加入一个小于号
     * （3）如果i+1插入一个小于号之间相当于同时加入一个大于号
     * （4）如果i+1插入一个大于号之间相当于同时加入一个小于号
     * （1）dp[i-1][j]将i加在开头，此时有了j个小于号
     * （2）dp[i-1][j-1]将i加在末尾，此时有了j-1个小于号
     * （3）dp[i-1][j]*j将j加在任意小于号之间，此时i-1个数有j个小于号
     * （4）dp[i-1][j-1]*（i-j-i）将i加在任意一个大于号之间，即i-1个数时有了j-1个小于号
