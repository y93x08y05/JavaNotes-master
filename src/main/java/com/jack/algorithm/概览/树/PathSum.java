...
public class PathSum {
    ...
    public static void main(String[] args) {
        ...
        int m = 9;
        Node root = pathSum.createTree(arr,0,arr.length-1);
        ArrayList<ArrayList<Integer>> arrayLists = pathSum.findPath(root,m);
    ...
    private ArrayList<ArrayList<Integer>> findPath(Node root,int target) {
        if (root == null) {
            return listAll;
        }
        list.add(root.value);
